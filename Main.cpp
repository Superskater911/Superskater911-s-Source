...
DWORD ScriptContext;
DWORD ScriptContextVFTable = x(0x1117F74); //edit
...
void PushGlobal(DWORD rL, lua_State* L, const char* s)
{
	...
	Bridge::push(rL, L, -1);
	MessageBoxA(NULL, "success 3", "success 3", NULL);
	lua_setglobal(L, s);
...
DWORD WINAPI input(PVOID lvpParameter)
{
	...
	char buffer[999999];
	DWORD dwRead;
	hPipe = CreateNamedPipe(TEXT("\\\\.\\pipe\\Luciferx"),
...
void main()
{
	...
	m_rL = ScriptContext + 56 * 0 + 164 + *(DWORD *)(ScriptContext + 56 * 0 + 164); //edit
	...
	MessageBoxA(NULL, "does pushglobal work?", "does pushglobal work?", NULL);
